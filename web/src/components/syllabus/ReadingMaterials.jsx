...
import { BookOpen, Clock, FileText, ArrowUpRight, Bookmark } from 'lucide-react';
import PixelBadge from '../common/PixelBadge';

export default function ReadingMaterials({ materials = [] }) {
  if (!materials || materials.length === 0) {
    return (
      <div className="p-8 text-center bg-slate-50 rounded-xl border-2 border-dashed border-slate-300">
        <BookOpen className="w-8 h-8 text-slate-400 mx-auto mb-2" />
        <p className="font-pixel text-xs text-ink font-bold">No reading materials found</p>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between pb-3 border-b-2 border-slate-200">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-pixel text-base font-bold text-ink">Reading Materials</h3>
        </div>
          ...
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-2">
                <span className="font-pixel text-[10px] font-bold text-primary uppercase tracking-wider">
                  {item.topic_id || item.domain || 'Core Concept'}
                </span>
                <PixelBadge variant="blue" size="sm">
                  {item.material_type || 'Article'}
                </PixelBadge>
              </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-2">
              <div className="flex items-center gap-1.5 text-xs text-ink-secondary">
                ...
                <span className="font-medium">{item.duration || '10 min read'}</span>
              </div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="pixel-btn-primary text-xs !py-1 !px-2.5 inline-flex items-center gap-1"
              >
