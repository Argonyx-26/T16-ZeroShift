...
import { Bookmark, Check, X, CheckCircle, XCircle } from 'lucide-react';
    ...
    <div className="bg-surface rounded-2xl border-2 border-slate-900 shadow-pixel p-4 sm:p-5 select-none">
      ...
      <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-5 lg:grid-cols-5 gap-2 mb-4">
        {Array.from({ length: totalQuestions }).map((_, idx) => {
          ...
          const isMarked = !isReviewMode && !!markedForReview[idx];

          let btnClass = "bg-slate-100 text-ink-secondary border-slate-300 hover:bg-slate-200";

          if (isReviewMode) {
            const q = questions[idx];
            const isCorrect = q && answers[idx] === q.correct_option_id;

            if (isCorrect) {
              btnClass = isCurrent
                ? "bg-learning-soft text-learning-hover border-learning font-bold ring-2 ring-learning ring-offset-1 shadow-pixel"
                : "bg-learning-soft text-learning-hover border-learning font-bold shadow-[1px_1px_0px_#16A34A]";
            } else {
              // Incorrect or skipped: mark as RED
              btnClass = isCurrent
                ? "bg-error-soft text-error border-error font-bold ring-2 ring-error ring-offset-1 shadow-pixel"
                : "bg-error-soft text-error border-error font-bold shadow-[1px_1px_0px_#EF4444]";
            }
          } else {
            if (isAnswered) {
      ...
      {isReviewMode ? (
        <div className="grid grid-cols-2 gap-2 pt-3 border-t border-slate-200 text-[10px] font-pixel text-ink-secondary">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-learning-hover font-bold">Correct</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded bg-error-soft border border-error" />
            <span className="text-error font-bold">Wrong / Skipped</span>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-2 pt-3 border-t border-slate-200 text-[10px] font-pixel text-ink-secondary">
